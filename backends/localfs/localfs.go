...
func (b LocalfsBackend) Put(key string, r io.Reader, expiry time.Time, deleteKey, accessKey string, srcIp string, maxDLs int64) (m backends.Metadata, err error) {
	filePath := path.Join(b.filesPath, key)
